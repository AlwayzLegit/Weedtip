-- ════════════════════════════════════════════════════════════════════════════
-- 20260705070000_delivery_address
-- Order-flow completion: delivery orders finally carry a delivery address.
-- - orders.delivery_address (jsonb: street/unit/city/state/zip/phone), written
--   only by create_order, which sanitizes and REQUIRES it for delivery orders.
-- - profiles.delivery_address stores the shopper's default for prefill.
-- create_order gains a parameter, so the old signature is dropped (a second
-- overload would make PostgREST named-arg calls ambiguous).
-- ════════════════════════════════════════════════════════════════════════════
...
drop function if exists public.create_order(uuid, order_type, jsonb, text, text, text, text);
...
begin
  ...
  -- Delivery orders must say where to go. Sanitize to a fixed key set so the
  -- column never stores arbitrary client JSON.
...
grant execute on function public.create_order(
  uuid, order_type, jsonb, text, text, text, text, jsonb
) to authenticated;
