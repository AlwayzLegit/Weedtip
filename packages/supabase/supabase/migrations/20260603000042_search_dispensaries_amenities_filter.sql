-- Facet filtering in the finder: when amenities are selected, a shop must have
-- ALL of them (Leafly-style AND faceting). Return type is unchanged; only a new
-- filter_amenities parameter + WHERE clause are added.
set search_path = public, extensions;

create index if not exists dispensaries_amenities_gin on public.dispensaries using gin (amenities);
...
returns table (
  ...
  city text, state char(2), zip text, phone text, email text, website text,
  ...
  latitude double precision, longitude double precision, status public.dispensary_status,
  featured boolean, rating_avg numeric, rating_count integer, created_at timestamptz,
  updated_at timestamptz, distance_meters double precision, is_open_now boolean,
...
grant execute on function public.search_dispensaries(
  text, double precision, double precision, double precision,
  boolean, boolean, boolean, boolean, boolean, text, text[], integer, integer
