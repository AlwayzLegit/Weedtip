-- ════════════════════════════════════════════════════════════════════════════
-- 20260722210000_checkout_kill_switch
--
-- Compliance kill-switch for consumer ordering/checkout. Our payment processor
-- requires that, under our current approval, the platform must NOT take orders
-- on behalf of dispensaries (we may market them and get paid for advertising).
-- Once bank approval lands, the super-admin flips this back on.
--
--   platform_settings.ordering_enabled  — single global toggle, default OFF.
--
-- Two enforcement layers ship with it:
--   1. This migration hard-guards create_order (the RPC the web AND mobile
--      clients call) so a direct API call is refused, not just the UI hidden.
--   2. App code (startCheckout action + every consumer CTA) reads the flag.
-- ════════════════════════════════════════════════════════════════════════════

alter table public.platform_settings
  add column if not exists ordering_enabled boolean not null default false;

comment on column public.platform_settings.ordering_enabled is
  'Global kill-switch for consumer online ordering/checkout. OFF = payment-processor compliant (marketing only, no order-taking). Toggled by super-admin.';

-- Guard create_order at the source. Reproduces the existing body with a single
-- new check at the very top: when ordering is globally disabled, no order can
-- be created regardless of caller (web, mobile, or a raw RPC call).
...
begin
  -- Compliance kill-switch: when online ordering is globally disabled, no order
  -- may be created by anyone. Enforced here so a direct RPC call can't bypass
  -- the hidden UI.
  if not coalesce((select ordering_enabled from public.platform_settings where id = 1), false) then
    raise exception 'Online ordering is currently unavailable on Weedtip.' using errcode = '22023';
  end if;
