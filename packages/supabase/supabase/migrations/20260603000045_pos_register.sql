-- Weedtip POS (add-on): real inventory counts + an in-store sale RPC. POS sales
-- write into the same orders pipeline (so GMV/analytics work) but carry no
-- platform commission and are marked completed+paid immediately.
set search_path = public;

alter table public.products add column if not exists stock_qty integer;

-- Ring up an in-store sale. Owner/admin only; prices server-authoritatively,
-- records a completed+paid order tagged source='pos' (zero commission), and
...
revoke all on function public.create_pos_order(uuid, jsonb, text) from public, anon;
grant execute on function public.create_pos_order(uuid, jsonb, text) to authenticated;
