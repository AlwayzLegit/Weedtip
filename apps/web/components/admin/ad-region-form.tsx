...
import { upsertAdRegion } from '@/app/admin/actions';
...
export function AdRegionForm({ region }: { region: Tables<'ad_regions'> | null }) {
    ...
    <form action={action} className="max-w-xl space-y-4">
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        <Field label="Name" htmlFor="name" error={fe.name}>
          <Input id="name" name="name" defaultValue={r?.name ?? ''} required placeholder="CA: Los Angeles" />
        </Field>
        ...
        <Field label="City" htmlFor="city" error={fe.city} hint="Optional — blank = statewide">
          <Input id="city" name="city" defaultValue={r?.city ?? ''} placeholder="Los Angeles" />
        </Field>
        <Field
        ...
        <Field label="Featured slots" htmlFor="slots" error={fe.slots}>
          <Input id="slots" name="slots" type="number" min="1" max="20" defaultValue={r?.slots ?? 1} />
        </Field>
      </section>
