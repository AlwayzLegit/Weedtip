...
export async function compSlot(_prev: FormState, fd: FormData): Promise<FormState> {
  ...
  revalidatePath('/admin/ad-regions');
  return formSuccess(`${shop.name} placed (comp/negotiated — no invoice generated).`);
}
...
/**
 * Flip a sales-led slot request live once billing is arranged. Service client:
 * ad_subscriptions has no client write policy — admin is verified first.
 */
