...
function isLive(p: { is_active: boolean; start_date: string | null; end_date: string | null }) {
  ...
  return (
    p.is_active && (!p.start_date || p.start_date <= today) && (!p.end_date || p.end_date >= today)
  );
}
...
export default async function DashboardPromos() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-muted mt-1 text-sm">
            Non-menu offers shown on your storefront — claimed in person. Up to 10 ({total}/10
            used).
          </p>
        </div>
        {total < 10 && (
      ...
      {!isPaid && (
        <UpgradeBanner message="In-store promos are a Weedtip Pro feature. Upgrade to publish them on your storefront." />
      )}
